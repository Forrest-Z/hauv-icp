...
/**
 * \brief align_submaps() function
 * \details Compute relative transformation between two submaps via the
 * Iterative Closest Point algorithm
 *
 * \param source The source pointcloud
 * \param target The target point cloud
 * \param transform The relative transform estimate (as a 4x4 matrix)
 * \param score The alignment score
 * \param initial_guess Initial guess for the relative transform
 * \param max_corresp_distance Point-to-point distance threshold
 * \return [description]
 */
...
int align_submaps_odom_prior(const typename pcl::PointCloud<PointT>::Ptr &source,
  ...
  ne_source.setInputCloud (source);
  ...
  ne_source.compute (*cloud_source_normals);
  
  const pcl::PointCloud<PointTNormal>::Ptr target_with_normals (new pcl::PointCloud<PointTNormal>);
  const pcl::PointCloud<PointTNormal>::Ptr source_with_normals (new pcl::PointCloud<PointTNormal>);
  ...
  pcl::concatenateFields (*source, *cloud_source_normals, *source_with_normals);
  ...
  // const typename pcl::registration::TransformationEstimationSVD<PointT, PointT, float>::Ptr tf_ptp( new pcl::registration::TransformationEstimationSVD<PointT, PointT, float>);
  ...
  icp.setTransformationEstimation(tf_ptp); 
  ...
  pcl::ConstCloudIterator<PointTNormal> source_it (*final);
